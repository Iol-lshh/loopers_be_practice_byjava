...
- 유스케이스
  ...
  - 주요 성공 시나리오
    ...
    - 5.서버는 주문 정보를 결제 대기중으로 저장한다.
    - 6.서버는 주문 생성 결과를 반환한다.
    - 7.클라이언트는 주문 생성 결과를 유저에게 보여준다.
  - 확장
    - 3a.유저가 존재하지 않는다면, NOT_FOUND 상태 코드를 반환한다.
    - 5a.주문의 상품 리스트에 존재하지 않는 상품이 있다면, NOT_FOUND 상태 코드를 반환한다.
    - 5b.주문의 상품 리스트에 재고가 부족한 상품이 있다면, BAD_REQUEST 상태 코드를 반환한다.
